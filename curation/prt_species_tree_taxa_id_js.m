...
%  check_entries; % test presence of entries in tree against local and server
  ...
  for i = 1:n
    ...
    fprintf(fid_tv, ['foldersTree = gFld(''<a id="node', node, '"><b>', node, '</b></a>'', "species_tree_',taxa{i},'.html?pic=', '%%22', node, '%%2Ejpg', '%%22")\n']);
    ...
    while length(pedigree_taxa) > 3
      ...
      if level == 1
        fprintf(fid_tv, ['L2 = insFld(foldersTree, gFld(''<a id="node', node, '">', node, '</a>'', "species_tree_',taxa{i},'.html?pic=', '%%22', node, '%%2Ejpg', '%%22"))\n']);
      ...
      elseif isempty(strfind(node, '_')) && isempty(strfind(node, ' ')) 
        fprintf(fid_tv, [Lnew, ' = insFld(', L, ', gFld(''<a id="node', node, '">', node, '</a>'', "species_tree_',taxa{i},'.html?pic=', '%%22', node, '%%2Ejpg', '%%22"))\n']);
