...
% The first-level fields are: climate, ecozone, habitat, migrate, food; the second-level fields are the codes
...
txt = url(strfind(url,'id="H.'): strfind(url,'id="M"')); j = 6 + strfind(txt, 'id="H.'); n = length(j);
for i = 1:n
  rtxt = txt(j(i):end);
  code = rtxt(1:strfind(rtxt,'"') - 1);
  rtxt(1:4 + strfind(rtxt,'</b> ')) = [];
